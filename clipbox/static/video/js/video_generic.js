...
//resive div heights for tags on page resize
...
$( "#slide_zoom" ).click(function() {
    ...
    if ($( "#display_slide" ).hasClass( "zoom_slide" )){
       $('#slide_zoom').html('<img src="/static/video/img/zoom-in.png"/>')

    } 
    else{
        $('#slide_zoom').html('<img src="/static/video/img/zoom-out.png"/>')
    }
...
//make all the page_tags the same height
    var highestBox = 0;
    $('.page_tags').each(function(){
...
function acceptDecline(member, response){
...
console.log(member + response);
    $.ajax({
        ...
        success : function(json){
            ...
            $(".membership-item-" + json.this_group).hide()
...
function slideToTag(id){
    ...
    var slideMove = $(".slide_id_" + id).clone()

    console.log(slideMove);
    slideMove.appendTo('#selected_slide');
