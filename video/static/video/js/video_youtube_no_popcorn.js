...
function updateTimerDisplay(){
    ...
    var video_percent = player.getCurrentTime() * (  100 / duration );
		...
		success : function(json){
			...
			$('#tags_on_load').prepend(
				"<div id=\"tag_" + json.tagpk  + " \" class=\"page_tags newtag user_"  + json.tag_user_id +  "  \" >" +
					"<div class=\"tag-left\">" +
						"<img class= 'image_comments' src='" + static_url +"video/uploaded_tag_images/default.png'>" + 
						"<div class=\"tag-time\">" + json.tag_start_string + "</div>"  +
						"<div class=\"tag_avatar\">" + 
							"<img class= \"user_avatar\" src=\"" + static_url + "video/" + json.user_avatar + "  \">" +
						"</div>"  +	
...
// bind mouse events to the custom time bar     - Drag status */
var timeDrag = false;    
...
/* Taking buffering out for now - not sure it really works witht the way youtube splits videos.
var startBuffer = function() {
	var buffered = player.getVideoLoadedFraction();
    console.log(buffered);
    //var maxduration = duration;
    //var currentBuffer = vid.buffered.end(0);
    var percentage = 100 * buffered / duration;
